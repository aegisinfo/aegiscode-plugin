 ...
 * ("  ⎿  $ {\"file_path\":\"src/hello.js\",...}"): you could see that a file
 * changed and never *what* changed — the one thing a coding session is
 * actually about. This module turns the tool call's arguments into a real
 * unified-style diff, which diffview.js paints as a colored block.
 *
 ...
 * no `process.stdout` — so the whole thing is unit-testable with no TTY. The
 * ANSI painting lives in diffview.js, exactly the split render.js already
 * keeps between structure and paint.
 *
 * Ported from `aegiscodex-dev/src/diff.js`; the engine's tool names are
...
// render path, so a 200 MB writeFile must not stall the CLI — the read is
...
function lineDiff(oldText, newText) {
  ...
  // would otherwise paint "… 46 unchanged lines" plus three orphans over an
  // identical pair — a marker claiming an omission that never happened.
...
function editPreview(name, args, opts = {}) {
  try {
    const cwd = opts.cwd == null ? process.cwd() : opts.cwd;
